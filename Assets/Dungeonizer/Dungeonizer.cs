...
public class Dungeonizer : MonoBehaviour {
    ...
    public List<string> customRoomType = new List<string>();
    public List<string> TypeGet()
    {
        return customRoomType;
    }
    public List<CustomRoom> ListGet()
    {
        return customRooms;
    }
    //public List<CustomRoom> customRooms = new List<CustomRoom>();
	...
	class Dungeon {
        ...
        private List<CustomRoom> customRooms = new List<CustomRoom>();
        private List<string> customRoomTypes = new List<string>();
		...
		public void Generate() {
			...
			for (var i = 0; i < room_count; i++) {
				...
				if (rooms.Count == 0) {
                    ...
                    for (int a = 0; a < customRooms.Count; a++)
                    {
                        if(customRooms[a].roomId == rooms.Count)
                        {

                            if (customRoomTypes[a] == "Treasure")                           //Size of treasure rooms
                            {
                                room.w = 10;
                                room.h = 5;
                            ...
                            else if (customRoomTypes[a] == "Shop")                          //Size of shops
                            {
                                room.w = 10;
                                room.h = 10;
                            ...
                            {
                                room.w = Random.Range(min_size, max_size);                  //random size
                                room.h = Random.Range(min_size, max_size);
			...
			for (int x = 0; x < map_size_x -1; x++) {
				for (int y = 0; y < map_size_y -1; y++) {
					if (map [x, y].type == 0) {
						if (map [x + 1, y].type == 1 || map [x + 1, y].type == 3) { //west 11
							map [x, y].type = 8;
							map [x, y].room = map [x + 1, y].room;
						}
						if (x > 0) {
							if (map [x - 1, y].type == 1 || map [x - 1, y].type == 3) { //east 9
								map [x, y].type = 8;
								map [x, y].room = map [x - 1, y].room;
						...
						if (map [x, y + 1].type == 1 || map [x, y + 1].type == 3) { //south 10
							map [x, y].type = 8;
			...
			for (int x = 0; x < map_size_x -1; x++) {
				for (int y = 0; y < map_size_y -1; y++) {
					if (walls.Contains (map [x, y + 1].type) && walls.Contains (map [x + 1, y].type) && roomsandfloors.Contains (map [x + 1, y + 1].type)) { //north 4
						map [x, y].type = 4;
						map [x, y].room = map [x + 1, y + 1].room;
					}
					if (y > 0) {
						if (walls.Contains (map [x + 1, y].type) && walls.Contains (map [x, y - 1].type) && roomsandfloors.Contains (map [x + 1, y - 1].type)) { //north 5
							map [x, y].type = 4;
							map [x, y].room = map [x + 1, y - 1].room;
					...
					if (x > 0) {
						if (walls.Contains (map [x - 1, y].type) && walls.Contains (map [x, y + 1].type) && roomsandfloors.Contains (map [x - 1, y + 1].type)) { //north 7
							map [x, y].type = 4;
							map [x, y].room = map [x - 1, y + 1].room;
					...
					if (x > 0 && y > 0) {
						if (walls.Contains (map [x - 1, y].type) && walls.Contains (map [x, y - 1].type) && roomsandfloors.Contains (map [x - 1, y - 1].type)) { //north 6
							map [x, y].type = 4;
							map [x, y].room = map [x - 1, y - 1].room;
		...
		for (int x = 0; x < Dungeon.map_size_x; x++) {
			for (int y = 0; y < Dungeon.map_size_y; y++) {
				if (Dungeon.map [x, y].type == 1 &&
					...
					if (Dungeon.map [x + 1, y].type == 1 && Random.Range(0,10) < 2 )
                    {                    
						location.byCorridor = true;
						location.asDoor = 1;                            // door? 4
						location.room = Dungeon.map[x + 1,y].room;

						spawnedObjectLocations.Add (location);
					}
					else if(Dungeon.map [x - 1, y].type == 1 && Random.Range(0, 10) < 2)
                    {
						location.byCorridor = true;
						location.asDoor = 1;                            // door? 2
						location.room = Dungeon.map[x - 1,y].room;

						spawnedObjectLocations.Add (location);	
					}
					else if (Dungeon.map [x, y + 1].type == 1 && Random.Range(0, 10) < 2)
                    {
						location.byCorridor = true;
						location.asDoor = 1;                            //door? 3
    ...
    public void GenerateCustomRooms()
    {
        for (int a = 0; a < customRooms.Count; a++)
        {
            if (customRoomType[a] == "Treasure") //Generate treasure-room
            {
                var x = Dungeon.rooms[customRooms[a].roomId].x;
                var y = Dungeon.rooms[customRooms[a].roomId].y;
                Instantiate(tresureChest, new Vector3(x + 5, y + 3, 0), Quaternion.identity);
              
            }
            if (customRoomType[a] == "Shop") //Generate shop
            {
                var x = Dungeon.rooms[customRooms[a].roomId].x;
                var y = Dungeon.rooms[customRooms[a].roomId].y;
                Instantiate(shopTable, new Vector3(x + 2, y + 8, 0), Quaternion.identity);
